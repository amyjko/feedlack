...
*/
	...
	$('#clusters').append(
		$("<p></p>").
		html("Feedlack found <span class='result big'>" + feedlack.clusters.length + "</span> place" + (feedlack.clusters.length > 1 ? "s" : "")+ " that appear to be missing feedback:"
		)
	);
	...
	if(feedlack.okayHandlers) {
	
		$('#clusters').append(
			$("<p></p>").
			html("Feedlack found <span class='success big'>" + feedlack.okayHandlers.length + "</span> places that appear to always produce feedback.")
		);
	
		for(var okayNumber = 0; okayNumber < feedlack.okayHandlers.length; okayNumber++) {

			var okayHandler = feedlack.okayHandlers[okayNumber];
			var newOkayHandlerView =
				$("<div></div>").
					addClass('cluster').
					addClass('selectable').
					data(okayHandler);

			newOkayHandlerView.append($('<span>').html('&#x2713;').addClass('checkmark').css('fontSize', 'large'));
			newOkayHandlerView.append($('<span>').html(okayHandler.event).addClass('code'));
			newOkayHandlerView.append($('<span>').html(" at "));
			newOkayHandlerView.append(
				$('<span>').
				addClass('location').
				html(getReadableLocation(okayHandler.location)).
				data(okayHandler)
			);
			newOkayHandlerView.append($('<span>').addClass('success').html(" always produces output"));
			
			$('#clusters').append(newOkayHandlerView);


			newOkayHandlerView.click(function() {
				var subject = $(this);	
				showOkayHandler(subject);
			});
...
function getClusterDescription(cluster) {
	...
	return "<span class='code'>" + functionDescription + "</span> " + " at " + getReadableLocation(location);
...
function createShortClusterView(cluster) {
	...
	newShortClusterView.append($('<span>').html('&#x2717;').addClass('xmark').css('fontSize', 'large'));
...
function showOkayHandler(okayHandlerView) {

	$('.cluster').removeClass('selected');
	okayHandlerView.addClass('selected');
	
	showLine(okayHandlerView);
	
	var detail = $('#detail');	
	detail.empty();

	detail.append($('<span>').html("All paths through this handler appear to <span class='success'>produce feedback</span>."));	
